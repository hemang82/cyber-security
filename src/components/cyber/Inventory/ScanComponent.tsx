...
export default function ScanComponent({ ScanHistory }: any) {
  ...
  const uniqueDomains = Array.from(new Set(ScanHistory?.map((item: any) => item.asset_name))).filter(Boolean);
  ...
  const filteredData = ScanHistory.filter((item: any) => {
    const matchesDomain = selectedDomain ? item.asset_name === selectedDomain : true;
